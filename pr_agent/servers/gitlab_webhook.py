...
def is_bot_user(data) -> bool:
    try:
        ...
        if any(indicator in sender_name for indicator in bot_indicators):
            ...
            return True
        # A bot's display name may not carry the indicator even when its username does
        # (e.g. name "Code Review" with username "code-reviewer"), so match on both.
        if sender_username and any(indicator in sender_username for indicator in bot_indicators):
